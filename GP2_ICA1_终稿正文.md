...
The main analysis used the pre-processed high-resolution single-cell RNA-seq files from GSE141259, generated from the lung injury study by Strunz et al. (2020). Raw count matrices and matched cell metadata were retrieved from GEO. The analysis focused on alveolar epithelial states relevant to AT2 differentiation: AT2, activated AT2, Krt8+ ADI, AT1 and Mki67+ proliferating cells. Cells were retained if they belonged to one of these states and passed quality filters (≥500 detected genes, ≥1,000 UMIs, ≤20% mitochondrial reads). The final analysis object contained 10,381 cells from 32,559 high-resolution cells in the parent object.
...
**Table 1. Dataset and analysis scope.**
...
| Analysis A states | Time points after injury; day 28 excluded from BioTIP scoring |
| Analysis B states | Five annotated alveolar cell types pooled across all time points |
...
When sampling time point was used as the state definition, BioTIP returned three statistically supported CTS candidates: day 5 (ΔIc.shrink = 1.10, 21 genes, p < 0.01), day 8 (ΔIc.shrink = 0.44, 23 genes, p = 0.01) and day 9 (ΔIc.shrink = 0.55, 30 genes, p < 0.01). For the day 5 module, Ic.shrink was highest at day 5 itself (Figure 6), and permutation testing confirmed that the observed ΔIc.shrink exceeded the gene-randomisation null (Figure 7). Overall, the time-point analysis places the strongest signals in mid-acute repair phases after widespread AT2 activation.

**Table 2. BioTIP CTS candidates (GSE141259).**
...
The timing of significant CTS candidates is biologically plausible relative to Strunz et al. (2020): early activated AT2 expansion precedes enrichment of Krt8+ ADI and later AT1-associated cells. The day 5 signal may therefore mark a phase in which the epithelial compartment shifts from widespread activation towards intermediate and differentiating states. Weaker or non-significant modules at sparse time points likely reflect cell-type mixing within each time label rather than absence of repair dynamics. Time-point states are useful for asking *when* the population appears most unstable, but they do not, by themselves, prove lineage direction or causal regulation.

### 3.8 Analysis B — cell type as state

When annotated cell type was used as the state definition, BioTIP identified three CTS modules: Krt8+ ADI (11 genes, ΔIc.shrink = 2.61, p = 0.01), AT1 (11 genes, ΔIc.shrink = 1.18, p = 0.04) and Mki67+ proliferation (22 genes, ΔIc.shrink = 1.00, p < 0.01). Ic.shrink for the Krt8+ ADI module peaked in that cell type (Figure 9), and permutation supported significance at p = 0.01 (Figure 10). MCI values were distributed across modules within each cell type, with prominent modules in Krt8+ ADI and proliferating cells (Figure 8). The Krt8+ ADI result aligns with the proposed transitional state between AT2 and AT1 (Strunz et al., 2020), whereas the proliferative module is consistent with a repair programme involving Mki67+ epithelial cells across time.
...
Despite caveats, the results are coherent with the published view of post-injury alveolar repair. The strongest time-point signals fall after early AT2 activation, while cell-type analysis highlights Krt8+ ADI and proliferative programmes. Together, these findings support a model in which injury triggers a temporally structured epithelial response with a pronounced transitional ADI state. BioTIP is therefore useful for highlighting candidate transition phases when combined with biological annotation, but is not a stand-alone substitute for careful cell-state interpretation or trajectory inference.
...
## Figures to insert in Word (from `figures_for_word/`)
