...
state <- as.character(seu$time_point)
...
writeLines(summary_lines, file.path(output_dir, "analysisA_simplified_run_summary.txt"))
...
saveRDS(subcounts, file.path(output_dir, "analysisA_simplified_subcounts.rds"))

igraphL <- getNetwork(subcounts, fdr = params$network_fdr)
saveRDS(igraphL, file.path(output_dir, "analysisA_simplified_networks.rds"))
...
saveRDS(cluster, file.path(output_dir, "analysisA_simplified_clusters.rds"))

membersL <- getMCI(cluster, subcounts, adjust.size = FALSE, fun = "BioTIP")
saveRDS(membersL, file.path(output_dir, "analysisA_simplified_membersL.rds"))
...
fwrite(cts_summary, file.path(output_dir, "analysisA_simplified_cts_summary.tsv"), sep = "\t")
...
saveRDS(
  ...
  file.path(output_dir, "analysisA_simplified_results.rds")
)
...
writeLines(
  c(
    ...
    "gate_status: pass",
    "checks:",
    "- simplified Analysis A completed using manifest-locked dataset",
...
message("Simplified Analysis A completed.")
